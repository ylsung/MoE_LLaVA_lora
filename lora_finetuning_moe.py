...
import os
os.environ["WANDB_DISABLED"] = "true" # TODO: added this line
...
IMAGE_ROOT = "/nas/longleaf/home/ylsung/work/LLaVA"


# def process_images(image, image_processor, config):
    
#     image_tensor = image_processor.preprocess(
#         Image.open(image).convert('RGB'), return_tensors='pt'
#     )['pixel_values']
...
def find_all_linear_names(model, add_keywords=None):
    ...
    multimodal_keywords = ['mm_projector', 'image_tower',
                           'video_tower', 'vision_resampler']
    
    if add_keywords is not None:
        multimodal_keywords = multimodal_keywords + add_keywords # TODO: Updated
...
def train(args_list, model, tokenizer, image_processor, data=None, lora_finetuning_modules=None):
    ...
    model_args, data_args, training_args = parser.parse_args_into_dataclasses(args=args_list) # TODO, updated here
    ...
    if model_args.moe_enable:
        if training_args.lora_enable:
            ...
            lora_config = LoraConfig(
                ...
                target_modules=lora_finetuning_modules if lora_finetuning_modules is not None else target_modules,
    ...
    # TODO: Init image processor
    if model.get_image_tower() is not None:
    ...
    if model.get_video_tower() is not None:
...
def test_on_one_sample(model, compute_dtype):    
    ...
    query = "What's the content of the image?"
    for img_path in [f"{IMAGE_ROOT}/images/australia.jpeg", f"{IMAGE_ROOT}/images/llava_logo.png"]:
        ...
        conv_mode = "stablelm"  # qwen or stablelm
        ...
        logits = model(
            input_ids=input_ids,
            images=img,
            return_dict=True, # TODO: added this line
        )["logits"]
        
        print(f"[Test Sample's logits] logits states, mean: {logits.mean():.3f}, std: {logits.std():.3f}")
        ...
        print(f"[Test Sample's outputs]", outputs)
    
    return outputs


def find_custom_linear_names(model, match_prefixes, postfix_exceptions=[], layers=None, experts=None):

    cls = torch.nn.Linear
    lora_finetuning_modules = set()
    for name, module in model.named_modules():
        # print(name)
        match = any(name.startswith(keyword) for keyword in match_prefixes) and not any(name.endswith(keyword) for keyword in postfix_exceptions)
        if match:
            if layers is None:
                # add all of layers
                if isinstance(module, cls):
                ...
                if any(f"layers.{i}." in name for i in layers):
                    if experts is None:
                        ...
                        if any(f"deepspeed_experts.{i}." in name for i in experts):
                            if isinstance(module, cls):
                                print("GET: ", name)
                                lora_finetuning_modules.add(name)
...
def easy_fine_tuning(
    ...
    args_list = (
        ...
        f" --moe_enable False"
        f" --lora_enable True"
        f" --version stablelm" # TODO: changed
        f" --data_path ./playground/data/llava_instruct_80k.json"
        f" --image_folder {image_folder}"
        # f" --vision_tower openai/clip-vit-large-patch14" # TODO: removed
        f" --bf16 {bf16}"
    ...
    # for LLaMA-7b layers: 
    # - prefix: model.layers.
    # - postfix_exception: ["wg"] this is like weight function which don't want to train them
    ...
    # for ViT layers: 
    # - prefix: model.image_tower.image_tower.
    ...
    #     model, match_prefixes=["model.image_tower.image_tower."], layers=[18],
    # )
    
    # Fine-tuning llama layers
    lora_finetuning_modules = find_custom_linear_names(
        model, match_prefixes=["model.layers."], postfix_exceptions=["wg"], layers=[18], experts=[2],
        # model, match_prefixes=["model.image_tower.image_tower."], layers=[18],
    )
...
if __name__ == "__main__":
    ...
    image_processor = processor['image'] # TODO: added this line
    model.router_aux_loss_coef = 0 # TODO: no moe loss
    ...
    sample_data = [
        {
            "id": "australia",
            "image": f"{IMAGE_ROOT}/images/australia.jpeg",
            "conversations": [
            {
                "from": "human",
